...
tape( 'the function has an arity of 3', function test( t ) {
	t.strictEqual( nanmaxBy.length, 3, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the maximum value of a one-dimensional ndarray via a callback function while ignoring `NaN` values', function test( t ) {
	var x;
	var v;

	x = [ 1.0, -2.0, NaN, 5.0, 0.0, 3.0 ];
	v = nanmaxBy( [ vector( x, 6, 1, 0 ) ], clbk );
	t.strictEqual( v, 10.0, 'returns expected value' );

	x = [ -4.0, -5.0 ];
	v = nanmaxBy( [ vector( x, 2, 1, 0 ) ], clbk );
	t.strictEqual( v, -8.0, 'returns expected value' );

	x = [ -0.0, 0.0, -0.0 ];
	v = nanmaxBy( [ vector( x, 3, 1, 0 ) ], clbk );
	t.strictEqual( isPositiveZero( v ), true, 'returns expected value' );

	x = [ NaN ];
	v = nanmaxBy( [ vector( x, 1, 1, 0 ) ], clbk );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = nanmaxBy( [ vector( x, 0, 1, 0 ) ], clbk );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = nanmaxBy( [ vector( x, 4, 2, 0 ) ], clbk );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = nanmaxBy( [ vector( x, 4, -2, 6 ) ], clbk );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = nanmaxBy( [ vector( x, 4, 2, 1 ) ], clbk );
	t.strictEqual( v, 8.0, 'returns expected value' );
